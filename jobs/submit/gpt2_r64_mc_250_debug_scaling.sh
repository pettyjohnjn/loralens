...
BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_r64_estimator_comparison_250_debug_scaling}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_r64_mc_250_debug_scaling}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_r64_estimator_comparison_250_debug_scaling}"
...
echo "Preparing GPT-2 LoRA r=64 Monte Carlo estimator run"
...
add_or_skip_run \
  "lora_subset_mc_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr1p5em3" \
  "${BASE_OUT}/lora_subset_mc_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr1p5em3" \
  "${COMMON_RUN_KV[@]}" \
  "LENS_TYPE=lora" \
  "LOSS_TYPE=subset_kl" \
  "LORA_RANK=${LORA_RANK}" \
...
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable Monte Carlo run was generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_r64_mc_250_debug_scaling.txt"
